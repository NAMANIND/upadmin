...
function MesheryPatterns({
    ...
    dataFetch(
      ...
      (result) => {
        if (result) {
          ...
          const patternsCatalogueCapability =
            capabilitiesRegistry?.capabilities.filter(
  ...
  const handleSetPatterns = (patterns) => {
    ...
    if (catalogVisibilityRef.current && catalogContentRef.current?.length > 0) {
      setPatterns([
        ...
        ...(patterns?.filter(
          (content) => content.visibility !== VISIBILITY.PUBLISHED
        ) || []),
      ]);
      return;
    }
  ...
  const handleVerify = (e, pattern_file, pattern_id) => {
    e.stopPropagation();
    const validationPayloads = generateValidatePayload(
      pattern_file,
      workloadTraitSet
    );
    ...
    dataFetch(
      ...
      (res) => {
        ...
        setPatternErrors(
          (prevErrors) => new Map([...prevErrors, [pattern_id, errors]])
        );
        handleModalOpen(
          e,
          pattern_file,
  ...
  const handlePublish = (formData) => {
    const compatibilityStore = _.uniqBy(meshModels, (model) =>
        ...
        if (result) {
          const filteredPatterns = result.patterns.filter((content) => {
            if (
              ...
              content.visibility === visibilityFilter
            ) {
              return true;
            }
            return false;
          ...
          return (
            ...
            >
              ...
              {visibility === VISIBILITY.PUBLISHED ? (
                ...
                <TooltipIcon
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    setSelectedPattern({
                      pattern: patterns[tableMeta.rowIndex],
                      show: true,
                    });
                  }}
                  disabled={
              ...
              <TooltipIcon
                ...
                title="Validate"
                onClick={(e) =>
                  handleVerify(e, rowData.pattern_file, rowData.id)
                }
                disabled={
                  !CAN(
                    keys.VALIDATE_DESIGN.action,
                    keys.VALIDATE_DESIGN.subject
                  )
                }
              >
              ...
              <TooltipIcon
                ...
                disabled={
                  !CAN(
                    keys.UNDEPLOY_DESIGN.action,
                    keys.UNDEPLOY_DESIGN.subject
                  )
                }
                onClick={(e) =>
                  handleModalOpen(
              ...
              <TooltipIcon
                ...
                disabled={
                  !CAN(
                    keys.DOWNLOAD_A_DESIGN.action,
                    keys.DOWNLOAD_A_DESIGN.subject
                  )
                }
              ...
              <TooltipIcon
                ...
                disabled={
                  !CAN(
                    keys.DETAILS_OF_DESIGN.action,
                    keys.DETAILS_OF_DESIGN.subject
                  )
                }
              ...
              {canPublishPattern && visibility !== VISIBILITY.PUBLISHED ? (
                <TooltipIcon
                  ...
                  disabled={
                    !CAN(
                      keys.PUBLISH_DESIGN.action,
                      keys.PUBLISH_DESIGN.subject
                    )
                  }
  ...
  async function showModal(count, patterns) {
    ...
    let response = await modalRef.current.show({
      ...
      subtitle: `Are you sure you want to delete the ${patterns} design${
        count > 1 ? "s" : ""
      }?`,
  ...
  const options = {
    ...
    onCellClick: (_, meta) =>
      meta.colIndex !== 3 && setSelectedRowData(patterns[meta.rowIndex]),
        ...
        <>
                    ...
                    <div className={classes.createButton}>
                      <div style={{ display: "flex", order: "1" }}>
                        <Button
                          ...
                          disabled={
                            !CAN(
                              keys.IMPORT_DESIGN.action,
                              keys.IMPORT_DESIGN.subject
                            )
                          }
                        >
                        ...
                        <Button
                          ...
                          disabled={
                            !CAN(
                              keys.CREATE_NEW_DESIGN.action,
                              keys.CREATE_NEW_DESIGN.subject
                            )
                          }
                        >
          ...
          <ConfirmationModal
            ...
            submit={{
              ...
              verify: () =>
                handleVerify(modalOpen.pattern_file, modalOpen.patternID),
            }}
          ...
          {importModal.open &&
            CAN(keys.IMPORT_DESIGN.action, keys.IMPORT_DESIGN.subject) && (
              <ImportModal
